...
module.exports.addSendMessage = async (req, res, next) => {
  try {
    ...
    if (userDetails.balance >= req.body.contact_no.length) {
      ...
      console.log("contact_numbers_str",contact_numbers_str);
...
const WhiteListNumbers=WhiteListNum. replace(/,*$/, '');
      ...
      // if (req.body.contact_no.length <= 20) {
      //   const res2 = await axios.get(
      //     `https://obd37.sarv.com/api/voice/voice_broadcast.php?username=${username}&token=${token}&plan_id=32782&announcement_id=${res1.data.data[0].announcement_id}&caller_id=0&contact_numbers=${WhiteListNumbers}&retry_json={"FNA":"1","FBZ":0,"FCG":"2","FFL":"1"}`
      //   );
      //   console.log("res2", res2);
      // }
